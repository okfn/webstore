...
class WebstoreTestCase(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        cls.app = ws.app.test_client()
        cls.engine = ws.app.db_factory.engine
        conn = cls.engine.connect()
        result = conn.execute(text(
            "select nspname from pg_namespace " 
            "where not nspname like 'pg_%' and " 
            "nspname not in ('public', 'information_schema')")).fetchall()
        for row in result:
            conn.execute('drop schema "%s" cascade' % row[0])
